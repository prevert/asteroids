...
class Hra(pyglet.window.Window):
    def __init__(self):
        ...
        self.score_label = pyglet.text.Label(text="Score:0 Max.score:0", x=10, y=10)
        self.score = 0
        self.highscore = 0
...
##        pyglet.clock.schedule_interval(lambda x: self.background_music.play(), 13.8)
    ...
    def game_tick(self, dt):
        ...
        if self.keys[pyglet.window.key.UP]:
            las = pyglet.sprite.Sprite(img=self.laser_image, x = self.ship.x , y = self.ship.y)
            las.scale = 1
            self.lasers.append(las)
            
        for laser in self.lasers:
            laser.y = laser.y + dt*500
            if laser.y > 600:
    ...
    def update_asteroids(self):
        ...
        for las in self.lasers :
            for asteroid in self.asteroids :
                if self.sprites_collide(las, asteroid):
                    self.asteroids.remove(asteroid)
                    self.explosion_sound.play()
    ...
    def update_score(self):
        ...
        self.score_label.text = "Score: %s Max.score: %s" % (self.score, self.highscore)
